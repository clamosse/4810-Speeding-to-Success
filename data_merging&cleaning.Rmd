...
double_mean_openess_and_combine <- right_join(
  ...
  y = scaled_data,
  by = c('nflId'),
) %>% filter(count >= 1) %>% select(-c('nflId','count','n_routes','displayName'))

colMeans(double_mean_openess_and_combine)

x_var <- double_mean_openess_and_combine %>% select(-c('nflId','count','n_routes','displayName','mean'))
y_var <- double_mean_openess_and_combine$mean

# using scaled combine stats
fit <- lm(mean ~ . - 1, data = double_mean_openess_and_combine)
fit
summary(fit)
